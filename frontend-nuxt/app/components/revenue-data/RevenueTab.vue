...
const breakoutParam = computed(() => {
  const map = { 'Source': 'source', 'Revenue Type': 'revenue_type', 'Commodity': 'commodity' }
  return map[selectedBreakout.value]
})

const { data: revenueData, refresh } = await useAsyncData(
  () => $fetch(`${apiUrl}/revenue-summary`, { query: { breakout: breakoutParam.value } }),
  { watch: [breakoutParam] }
)
...
const chartData = computed(() => {
  ...
  if (isMonthly) {
    ...
    if (isFiscalYear) {
      ...
      monthRows = revenue.value.filter(row => row.fiscal_year === fy)
    } else if (isCalYear) {
      const cy = parseInt(selectedPeriod.value.replace('Calendar Year ', ''))
      monthRows = revenue.value.filter(row => row.calendar_year === cy)
    } else {
      const periodDates = [...new Set(revenue.value.map(r => r.period_date).filter(Boolean))].sort()
      const recent12 = periodDates.slice(-12)
      monthRows = revenue.value.filter(row => recent12.includes(row.period_date))
    }
  ...
  const grouped = {}
  const breakouts = new Set()
  for (const row of revenue.value) {
...
const comparisonTable = computed(() => {
  ...
  const fiscalYears = [...new Set(revenue.value.map(r => r.fiscal_year).filter(Boolean))].sort()
  ...
  const fy1Months = [...new Set(revenue.value.filter(r => r.fiscal_year === fy1).map(r => r.fiscal_month))].sort((a, b) => a - b)
  const isPartial = fy1Months.length < 12

  const lastMonthRow = revenue.value.find(r => r.fiscal_year === fy1 && r.fiscal_month === fy1Months.at(-1))
  const firstMonthRow = revenue.value.find(r => r.fiscal_year === fy1 && r.fiscal_month === fy1Months[0])
...
<template>
  ...
  <div class="display-flex flex-row flex-align-end">
    ...
    <div class="padding-bottom-05 margin-right-2 flex-1">
      <label class="usa-label margin-top-05" for="revenue-period-select">Period</label>
      <select class="usa-select" name="revenue-period-select" id="revenue-period-select" v-model="selectedPeriod">
        <option v-for="option in periodOptions" :key="option">{{ option }}</option>
      </select>
    </div>
    <div class="padding-bottom-05 flex-1">
      <label class="usa-label margin-top-05" for="revenue-breakout-select">Breakout</label>
      <select class="usa-select" name="revenue-breakout-select" id="revenue-breakout-select" v-model="selectedBreakout">
