...
| `clear-fy-disbursements.py` | Delete one fiscal year's `disbursement` rows on its `period.type='Fiscal Year'` row (leaves the period + the Monthly rows), so the revenue-data-update hook's create-once FY summarizer re-creates them on the next ingest — a testing helper. Dry-run by default; `--apply` to delete. `TOKEN=<admin> python3 cms/seed/clear-fy-disbursements.py <year> [--apply]`. |
| `clear-month-disbursements.py` | Delete one month's `disbursement` rows on its `period.type='Monthly'` row (leaves the period + dimensions; other months untouched), so a month loaded twice/with pre-fix logic can be re-loaded cleanly. Does not refresh FY rollups — see `clear-fy-disbursements.py`. Dry-run by default; `--apply` to delete. `TOKEN=<admin> python3 cms/seed/clear-month-disbursements.py <YYYY-MM> [--apply]`. |
...
| `seed-fy-disbursements-dictionary.sql` | Seed the Fiscal Year Disbursements `data_dictionary` by copying the Monthly disbursements dictionary (identical columns) to the FY dataset with fresh ids, plus an FY-specific reword of the leading `Date` field. Resolves both datasets by name (`source_collection` is shared). Re-running clears + re-copies. Run: `psql "$DB_URL" -f cms/seed/seed-fy-disbursements-dictionary.sql`. |
| `seed-federal-revenue-by-company-dictionary.sql` | Seed the Federal Revenue by Company `data_dictionary` (6 fields: Calendar Year, Corporate Name, Revenue Agency, Revenue Type + values, Commodity + values, Revenue) and the `data_dictionary_intro`. Resolves the dataset by `source_collection='federal_revenue_by_company'`, so it's portable; re-running clears + re-seeds. Run: `psql "$DB_URL" -f cms/seed/seed-federal-revenue-by-company-dictionary.sql`. |
