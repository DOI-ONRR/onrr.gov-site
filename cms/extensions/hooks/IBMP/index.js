...
const parseFile = (payload, file) => {
  return readXlsxFile(file, { 
    ...
    const fDate = new Date(`${ rows[0][0] }, ${ getMonthFromString(rows[0][1], rows[0][0]) }, 01`);
...
module.exports = function ibmp({ filter, action }) {
  filter('ibmp.items.create', async (payload, meta,context) => {
