...
const parseFile = (payload, file) => {
  return readXlsxFile(file, { 
    ...
    const fDate = new Date(`${ rows[0][1] }, ${ getMonthFromString(rows[0][2], rows[0][1]) }, 01`);
...
module.exports = function index_gas_major_portion({ filter, action }) {
  filter('indian_gas_major_portion.items.create', async (payload, meta,context) => {
