/**
 * Filters out fiscal year production records where all key fields are empty/null.
 * Returns null if all fields are empty.
 *
 * Based on: ignore_empty_production_fy.sql
 *
 * @param {Object} record - The production record
 * @returns {Object|null} - The record, or null if it should be skipped
 */
export function ignoreEmptyProductionFY(record) {
  const isEmpty = (value) => value === null || value === undefined || value === '';

  if (
    isEmpty(record.fiscal_year) &&
