export default class CollectionsTool {
  ...
  render () {
    ...
    selectAccordionInputLabel.setAttribute(
      'for',
      'collectionsAccordionSelector'
    )
    ...
    this.fetchCollections().then(collections => {
      ...
      if (this.data && this.data.collection) {
        const foundIndex = Array.from(selectInput.options).findIndex(
          item => item.value === this.data.collection
        )
    ...
    this.fetchContacts().then(categories => {
      ...
      if (this.data && this.data.collection) {
        if (this.data.page) {
          ...
          const foundIndex = Array.from(selectPageInput.options).findIndex(
        ...
        if (this.data.tab) {
          ...
          const foundIndex = Array.from(selectTabInput.options).findIndex(
            item => item.value === tab
          )
        ...
        if (this.data.accordion) {
          ...
          const foundIndex = Array.from(selectAccordionInput.options).findIndex(
            item => item.value === accordion
          )
        ...
        if (this.data.topics && this.data.topics.length > 0) {
          this.data.topics.forEach(topic => {
            let t = topic.replace('&amp;', '&')
            Array.from(selectTopicInput.options).find(
              o => o.value == t
            ).selected = true
          })
        }
    ...
    if (this.data && this.data.layout) {
      const foundIndex = Array.from(selectLayoutInput.options).findIndex(
        item => item.value === this.data.layout
      )
      selectLayoutInput.options[foundIndex || 0].selected = true
    }

    if (this.data && this.data.status) {
      const foundIndex = Array.from(selectStatusInput.options).findIndex(
        item => item.value === this.data.status
      )
    ...
    selectInput.addEventListener('change', e => {
      ...
      if (
        e.target.value === 'press_releases' ||
        e.target.value === 'reporter_letters'
      ) {
    ...
    if (this.data && this.data.collection) {
      ...
      if (
        ...
        this.data.collection !== 'reporter_letters' ||
        this.data.collection !== 'press_releases'
      ) {
    ...
    selectFieldsCol7.style.display =
      this.data && this.data.collection === 'reporter_letters'
        ? 'block'
        : 'none'
  ...
  save (blockContent) {
    ...
    const selectLayout = blockContent.querySelector(
      '#collectionsLayoutSelector'
    )
    const selectStatus = blockContent.querySelector(
      '#collectionsStatusSelector'
    )
    ...
    const contactAccordion = blockContent.querySelector(
      '#collectionsAccordionSelector'
    )
    const selectedTopics = blockContent.querySelectorAll(
      '#collectionsTopicSelector option:checked'
    )
